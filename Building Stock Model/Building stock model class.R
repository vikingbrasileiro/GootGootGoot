...
p_prepare_data <- function() 
{
  ...
  self$ls_inputs_data$insulation_policy = self$data_raw %>% 
    ...
    select( Building_Class = ID_Item, Scenario, Sector, Usage, Unit, Year, Value) %>%
...
p_project_efficiency_main_RES <- function()
{
  ...
  insulation_new = self$ls_inputs_data$insulation_policy %>%
    ...
    select( Scenario, Sector, building_class, construction_year, insulation_new )
  ...
  ############## Need to extend insulation and efficiency value for old building ###########################
  
  
  equipments = unique( insulation_old$Equipment )
  scenarios = unique(insulation_new$Scenario) ### Need to be changed when the excel input files will be updated
  ...
  equipment_stock = equipment_stock %>%
    ...
    left_join( insulation_new , by = c( "Scenario", "Sector", "building_class", "construction_year" ) ) %>%
...
p_project_KPI <- function()
{
  ...
  #8. primary consumption per sqm per building class
  ...
  ls_KPI = list( 
    ...
    "insulation_thermal_per_bc" = insulation_per_bc,
...
setwd("C:/Users/JB5872/Documents/1.12 Building Stock/0. Data/")

data = readr::read_delim( "C:/Users/JB5872/Documents/1.12 Building Stock/0. Data/test_data_7.csv" , delim = ";")
country_mapping = readr::read_delim( "C:/Users/JB5872/Documents/1.12 Building Stock/0. Data/CountryRegionMapping.csv" , delim = ",")
policy = readr::read_delim( "C:/Users/JB5872/Documents/4. R file/GIT/GootGootGoot/Building Stock Model/Data/Inputs/PolicyData.csv" , delim = ",")

data = data %>% filter(ID_Item != "Primary energy renovated",ID_Item != "Primary energy new" )
data = rbind(data,policy)
